...
def plot_feature_importance(bdt_best, outpath="bdt/feature_importance.png"):
    ax = xgb.plot_importance(bdt_best, max_num_features=100, importance_type="gain",values_format='{v:.2f}')
...
def optimize_sig(group_of_samples,channel,n=39):
    strength=[]
    unc = []
    for i in range(n):
    ...
    plt.tight_layout()
    #plt.show()
    plt.savefig(f"bdt/optimize_bdt_{channel}.png")
def main():
