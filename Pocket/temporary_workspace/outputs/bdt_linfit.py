...
def run(cfg,use_calib=False):
    ...
    for feat in feature_names:
        ...
        # ── Weighted linear fit on the nominal Data/MC BDT score ratio ────────
        # Fit:  ratio(score) = a * score + b
        # Weights: 1 / sigma^2  per bin
        fit_result = None
        x_line = y_line = y_line_up = y_line_down = None
        fit_mask = (
            np.isfinite(r_nom) &
            np.isfinite(err_r_nom) &
            (err_r_nom > 0)
        )
        if fit_mask.sum() >= 2:
            x_fit = score_centres_1d[fit_mask]
            y_fit = r_nom[fit_mask]
            w_fit = 1.0 / err_r_nom[fit_mask] ** 2
 
            W   = w_fit.sum()
            Wx  = (w_fit * x_fit).sum()
            Wx2 = (w_fit * x_fit ** 2).sum()
            Wy  = (w_fit * y_fit).sum()
            Wxy = (w_fit * x_fit * y_fit).sum()
            det = W * Wx2 - Wx ** 2
 
            if abs(det) > 0:
                a = (W  * Wxy - Wx * Wy)  / det   # slope
                b = (Wx2 * Wy - Wx * Wxy) / det   # intercept
                var_a  =  W   / det
                var_b  =  Wx2 / det
                if use_calib:
                    a=0.3906
                    ...
                    var_a=0.0429**2
                    var_b=0.0179**2
                fit_result = dict(slope=a, intercept=b,
                                    slope_err=np.sqrt(var_a),
                                    intercept_err=np.sqrt(var_b))
    
                x_line      = np.linspace(score_edges_1d[0], score_edges_1d[-1], 200)
                y_line      = a * x_line + b
                y_line_up   = (a + np.sqrt(var_a)) * x_line + (b + np.sqrt(var_b))
                y_line_down = (a - np.sqrt(var_a)) * x_line + (b - np.sqrt(var_b))
 
                print(f"    Linear fit Data/MC vs BDT score: "
                      f"slope={a:.4f}+/-{np.sqrt(var_a):.4f}  "
                      f"intercept={b:.4f}+/-{np.sqrt(var_b):.4f}")
            else:
                print("    WARNING: singular matrix, skipping linear fit.")
        else:
            print("    WARNING: too few valid ratio bins for a linear fit.")
        ...
        if fit_result is not None:
            ax_ratio.plot(x_line, y_line, color="darkgreen", linewidth=1.5,
                          label=f"Linear fit (slope={fit_result['slope']:.3f})")
            ax_ratio.fill_between(x_line, y_line_down, y_line_up,
        ...
        # ══ FIGURE 4: Calibrated BDT score — MC reweighted by linear fit ══════
        # Apply correction:  w_cal = w / (a*score + b)
        # This makes the fitted ratio exactly 1 everywhere by construction.
        if fit_result is not None:
            a, b = fit_result["slope"], fit_result["intercept"]
            mc_scores     = df_mc["bdt_score"].values
            correction    = a * mc_scores + b
            correction    = np.where(correction > 0, correction, 1.0)  # guard
            mc_weight_cal = df_mc["weight"].values * correction
 
            h_mc_cal, err_mc_cal = whist(
                df_mc["bdt_score"], mc_weight_cal, score_edges_1d
            )
 
            with np.errstate(divide="ignore", invalid="ignore"):
                r_cal = np.where(h_mc_cal > 0, h_data_base / h_mc_cal, np.nan)
                err_r_cal = np.where(
                    h_mc_cal > 0,
                    np.sqrt((err_data_base / h_mc_cal) ** 2
                            + (h_data_base * err_mc_cal / h_mc_cal ** 2) ** 2),
                    np.nan,
                )
            ...
            ax4_main, ax4_ratio = axes4
 
            ax4_main.errorbar(score_centres_1d, h_data_base, yerr=err_data_base,
                              fmt="o", color="black", markersize=3, linewidth=1.0,
                              label="Data", zorder=5)
            ax4_main.step(score_centres_1d, h_mc_base, where="mid",
                          color="steelblue", linewidth=1.5, linestyle="--",
                          label="MC (nominal)")
            ax4_main.fill_between(score_centres_1d,
                                  h_mc_base - err_mc_base,
                                  h_mc_base + err_mc_base,
                                  step="mid", alpha=0.3, color="steelblue")
            ax4_main.step(score_centres_1d, h_mc_cal, where="mid",
                          color="darkgreen", linewidth=1.5,
                          label="MC (linear calibration)")
            ax4_main.fill_between(score_centres_1d,
                                  h_mc_cal - err_mc_cal,
                                  h_mc_cal + err_mc_cal,
                                  step="mid", alpha=0.3, color="darkgreen")
            ax4_main.set_ylabel("Normalised events / bin")
            ax4_main.set_title(
                f"BDT score: linear calibration [{feat}]\n"
                f"slope={a:.4f}+/-{fit_result['slope_err']:.4f}  "
                f"intercept={b:.4f}+/-{fit_result['intercept_err']:.4f}"
            )
            ax4_main.legend(fontsize=9)
 
            ax4_ratio.errorbar(score_centres_1d, r_nom, yerr=err_r_nom,
                               fmt="s", color="steelblue", markersize=3,
                               linewidth=1.0, linestyle="--", label="Data/MC nom.")
            ax4_ratio.errorbar(score_centres_1d, r_cal, yerr=err_r_cal,
                               fmt="o", color="darkgreen", markersize=3,
                               linewidth=1.0, label="Data/MC calibrated")
            ax4_ratio.plot(x_line, y_line, color="darkgreen",
                           linewidth=1.2, linestyle=":", label="Linear fit")
            ax4_ratio.axhline(1.0, color="black", linewidth=0.8, linestyle=":")
            ax4_ratio.set_ylim(0.5, 1.5)
            ax4_ratio.set_ylabel("Data / MC")
            ax4_ratio.set_xlabel("BDT score")
            ax4_ratio.legend(fontsize=8)
 
            plt.tight_layout()
            fig4.savefig(os.path.join(cfg["output"],
                                      f"score_calibration_{feat}.png"),
                         dpi=150, bbox_inches="tight")
            plt.close(fig4)
            print(f"    Saved calibration plot: score_calibration_{feat}.png")
...
def main():
    ...
    parser.add_argument(
        "--use_calib",default=False, help='rederive slope or no?'
    )
