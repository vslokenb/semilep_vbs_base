...
def select_QCD_CR(events, params, **kwargs):
    if "WJetsToLNu_TuneCP5_13TeV-madgraphMLM-pythia8" in events.metadata["dataset"]:
        ...
        ht_mask = True
    one_lep = (events.nMuonLoose ==1) #### CHANGED FOR DEBUGGING
    veto = (events.nLeptonVeto < 2)
    if params["recoil"] == 'false':
        recoil_jet = True
        recoil_jet_pt = True
    else:
    ...
    if params["bveto"] == "true":
        bveto = (events.nBJetTight     ==  0)
    else:
        bveto = True
    mask = one_lep & met_cut & recoil_jet & cut_mt_w & recoil_jet_pt & cut_njet & ht_mask & veto & bveto
    return ak.values_astype(mask, np.bool_)
...
qcd_enriched_cut_0_deepmet_resolution_3j = Cut(
    name="qcd_enriched",
    params={
        "met_pt": 30.0,
        "recoil_jet_pt": 35,
...
qcd_enriched_cut_35_deepmet_resolution_3j_bveto = Cut(
    name="qcd_enriched",
    params={
        "met_pt": 30.0,
        "recoil_jet_pt": 35,
...
qcd_enriched_cut_40_deepmet_resolution_3j_bveto = Cut(
    name="qcd_enriched",
    params={
        "met_pt": 30.0,
        "recoil_jet_pt": 40,
...
qcd_enriched_cut_30_deepmet_resolution_3j_bveto = Cut(
    name="qcd_enriched",
    params={
        "met_pt": 30.0,
        "recoil_jet_pt": 30,
...
def select_QCD_validate(events, params, **kwargs):
    ...
    if params["met_def"] == "deepmet_response":
        ...
        met_cut = (events.PuppiMET.pt      <  params["met_pt"])
    mask = one_lep & cut_jet & loose_lep_veto & met_cut & ht_mask
    return ak.values_astype(mask, np.bool_)
