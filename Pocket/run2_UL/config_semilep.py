...
cfg = Configurator(
    ...
    categories={
        ...
        #"narrow_whad_window": [narrow_whad_window_cut], # same as above, but force both jets from hadronic resolved W to go to AK15
    ...
    variables={
        ...
        "flav_jet_hadron":       HistConf([Axis(coll="GenJet", field="hadronFlavour", bins=16, start=-8, stop=8, label="N(gen b jet (hadron))")]),
        "flav_jet_parton":       HistConf([Axis(coll="GenJet", field="partonFlavour", bins=60, start=-30, stop=30, label="N(gen b jet (parton))")]),
        "btagDeepFlavB":    HistConf([Axis(coll="Jet", field="btagDeepFlavB", bins=20, start=0, stop=1, label="deepJet discrim score")]),
        ...
        "ak8_ak4_separation":       HistConf([Axis(coll="events", field="ak8_ak4_separation", bins=40, start=0.0, stop=4.0, label=r"$\Delta R(AK8 to j)$")]),
        ...
        "bjet_pt":    HistConf([Axis(coll="BJetGood", field="pt", bins=60, start=0, stop=300, label=r"$p_T(b)$ [GeV]")]),
        "bjet_eta":   HistConf([Axis(coll="BJetGood", field="eta", bins=48, start=-4.8, stop=4.8, label=r"$\eta(b)$")]),
        "bjet_phi":   HistConf([Axis(coll="BJetGood", field="phi", bins=48, start=-4., stop=4., label=r"$\phi(b)$")]),
